// Original MODULE https://www.npmjs.com/package/expresstrade
...
var ExpressTrade = (() => {
  ...
  function ExpressTrade(options){
    ...
    // Check for Options
    if(typeof options !== "object") return console.log("OP-ExpressTrade - api key, secret and poll interval is missing. Check documentation.");
    if(typeof options.apikey !== "string") return console.log("OP-ExpressTrade - please provide a valid api key. Check documentation.");
    ...
    function startPolling(_interval){
      ...
      that.polling = setInterval(() => {
        that.ITrade.GetOffers((err, res) => {
          ...
          if(res && res.status != 1) return [ console.log("OP-ExpressTrade - " + res.status + ": " + res.message), clearInterval(that.polling) ];
        
          else if(!res.response || !res.response.offers) return console.log("OP-ExpressTrade - Failed for checking offers. Error: " + res.message);
    ...
    this.request = (_path, _data, _callback) => {
      ...
      if(typeof this.schema[_interface][_method].fields === 'object'){
        ...
        for(var field in this.schema[_interface][_method].fields) if(this.schema[_interface][_method].fields[field] === 1 && !data[field]) return console.log("OP-ExpressTrade - " + _path + ": Missing input '" + field + "'");
      }
    ...
    this.generateToken = (i = 0) => {
      ...
      if(!this.options.twofactorsecret) return console.log("OP-ExpressTrade - please provide a valid 2fa secret. Check documentation.");

      if(i > 10) return console.log("OP-ExpressTrade - couldn\'t generate the 2fa token.");
